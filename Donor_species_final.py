    ...
    if len(donor) == 2: # BN10 donors
        ...
        if database_files != []:
            ...
            try:
                ...
                if sub_samples != []:
                    cmds_merge += '%s mpileup --threads %s -a FMT/ADF,FMT/ADR,FMT/AD -q30 -Ou -B -d3000 -f %s %s  | %s call  --threads %s -m > %s.raw.vcf\n' % (
                        'bcftools', min(40, 40), database,
                        ' '.join(sub_samples), 'bcftools', min(40, 40), tempbamoutput)
...
def windowing_sub(CHR,POS_set,NS_set,windowsize):
    try:
        total_length = CHR.split('size')[1]
    except IndexError:
        total_length = CHR.split('length_')[1].split('_cov')[0]
    total_length = int(total_length)
    total_interval = int(total_length/windowsize)
    total_NS = ['']*(total_interval+1)
    for i in range(0,len(POS_set)):
        POS = POS_set[i]
        loci_POS = int(POS/windowsize)
        if total_NS[loci_POS] == '':
...
def windowingold(Seq_N):
    for windowsize in windowsize_set:
        N_S_set = [0,0,0]
        for CHR in Seq_N:
            total_NS = windowing_sub(CHR, Seq_N[CHR][0],
                                     Seq_N[CHR][1], windowsize)
            N_S_set[0] += total_NS.count('N')
            N_S_set[1] += total_NS.count('S')
        try:
            N_S_set[2] = N_S_set[0]/N_S_set[1]
