...
Round = args.rd
input_script_vcf = args.s + '/vcf_round_cluster'
input_script = args.s
genome_root = args.i
...
max_sample = 100 # max number of isolates per vcf
...
def runspades(file1,file2,temp_output,output_name):
    cmds = 'py37\n%s --careful -1 %s -2 %s -o %s --threads 40 --memory 100 --cov-cutoff 7\n' % \
...
def merge_sample_subcluster(database,vcfoutput,allsam,coverage_only = False):
    ...
    if len(allsam) <= max_sample:
        ...
        total_folder = int(len(allsam) / max_sample)
        ...
        for i in range(0, total_folder + 1):
            ...
            if i == total_folder:
                try:
                    ...
                    cmds += '%s mpileup --threads %s -a FMT/ADF,FMT/ADR,FMT/AD -q30 -Ou -B -d3000 -f %s %s | %s call -c -Ov --threads %s > %s.raw.vcf\n' % (
                        args.bcf, min(40, args.t), database,
                        ' '.join(allsam[i * max_sample:]), args.bcf, min(40, args.t), vcfoutput2)
                if not coverage_only:
                    try:
                ...
                try:
                    ...
                    cmds += '%s mpileup --threads %s -a FMT/ADF,FMT/ADR,FMT/AD -q30 -Ou -B -d3000 -f %s %s | %s call -c -Ov --threads %s > %s.raw.vcf\n' % (
                        args.bcf, min(40, args.t), database,
                        ' '.join(allsam[i * max_sample:(i + 1) * max_sample]), args.bcf, min(40, args.t), vcfoutput2)
                if not coverage_only:
                    try:
                        f1 = open('%s.flt.snp.vcf' % (vcfoutput2))
                    except FileNotFoundError:
...
def mapping_WGS(donor_species,donor_species_fastqall):
    ...
    if filesize == 0:
        ...
        if len(donor_species_fastqall) >= cluster_cutoff:
            qualifygenome = donor_species_fastqall
            species = donor_species.replace('PB','PaDi')
            ...
            for fastq_file in donor_species_fastqall:
                if '.all' + fastq_name not in fastq_file and '.all.spades' not in fastq_file:
                    ...
                    allsam += [results[1]]
                    cmds += results[0]
            cmds += merge_sample_subcluster(donor_species_genomename, alloutputvcf, allsam, False)
...
Ref_set = dict()
if args.ref != 'None':
    allref = glob.glob(args.ref + '/*')
    for folder in allref:
        donor_species = os.path.split(folder)[-1]
        species = donor_species.split('_')[0]
        genomefiles = glob.glob('%s/*.all.spades1.fasta.noHM.fasta'%(folder))
        if genomefiles!= []:
            Ref_set.setdefault(species,genomefiles[0])
        else:
            print(folder)
...
for folder in species_folder:
    donor_species = os.path.split(folder)[-1]
    donor,species = donor_species.split('_')
    if donor_species.startswith('P') or donor_species.startswith('H') or donor_species.startswith('D'):
        # IBD
        donor_species_fastqall = glob.glob(
            '/scratch/users/mit_alm/IBD_Evo/%s/%s/*/sickle2050/filter_reads_1.fastq' % (species,donor)) + \
                                 glob.glob(
                                     '/scratch/users/mit_alm/IBD_Evo/%s/*_%s/*/sickle2050/filter_reads_1.fastq' % (
                                     species, donor))
    else:
    ...
    if donor_species_fastqall == []:
        print('no fastq for %s'%(donor_species))
    species = species.replace('PB', 'PaDi')
    allfastq.setdefault(species, [])
    allfastq[species] += donor_species_fastqall

# generate code
for species in allfastq:
    print(species,allfastq[species])
    mapping_WGS(species, allfastq[species])

# sum all codes
f1 = open(os.path.join(input_script, 'allWGS_cluster.sh'), 'w')
f1.write('#!/bin/bash\nsource ~/.bashrc\n')
...
f1.close()
print('please run: sh %s/allWGS_cluster.sh'%(input_script))
################################################### END ########################################################
