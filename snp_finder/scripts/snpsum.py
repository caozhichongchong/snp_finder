...
annotation_file_prokka = '/scratch/users/anniz44/genomes/donor_species/vcf_round2/merge/details/summary/all.denovo.gene.faa.prokka.txt'
annotation_file_kegg = '/scratch/users/anniz44/genomes/donor_species/vcf_round2/merge/details/summary/all.denovo.gene.faa.cluster.aa.all.kegg.sum'
...
for lines in open(annotation_file,'r'):
    if not lines.startswith('cluster'):
        lines_set = lines.split('\n')[0].split('\t')
        newgene,eggnog,anno,cog,cog1,cog2,species_num,cross_genus = lines_set[4:12]
        annotation_set.setdefault(newgene,'%s\t%s\t%s\t%s\t%s'%(eggnog,anno[:40],cog1,cog2,cross_genus))
...
for snpfile in allsnpfiles:
    ...
    POS_all_0 = 0
    oldCHR = ''
    vcf_file = '%s/%s.raw.vcf.filtered.vcf.final.snp.txt'%(os.path.split(snpfile)[0],donorspecies.replace('.donor','.all.donor').replace('_PaDi_','_PB_'))
    gene_snp = dict()
    try:
        ...
        for lines in open(snpfile, 'r'):
            ...
            oldgene = lines_set[5]
            if oldgene == 'Gene':
                lines = 'Newgene\tSNP\tGene_length\tProkka_gene\tProkka_COG\tProkka_product\tEggnog\tAnno\tCOG1\tCOG2\tcore\tKO\tKO1\tKO2\tKO3\tPOSnew\t' + lines
            else:
                CHR = lines_set[0]
                POS = int(lines_set[1])
                ...
                annoall += '\t' + annotation_set.get(newgene, 'None\tNone\tNone\tNone\tNone')
        ...
        print('can not find %s'%(vcf_file))

os.system('mkdir  /scratch/users/anniz44/genomes/donor_species/vcf_round2/merge/details/snpsummary/')
os.system('mv /scratch/users/anniz44/genomes/donor_species/vcf_round2/merge/details/*snpsum.txt /scratch/users/anniz44/genomes/donor_species/vcf_round2/merge/details/snpsummary/')
