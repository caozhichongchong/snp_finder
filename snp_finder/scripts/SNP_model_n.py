...
def run_mapper(files,files2,database,tempbamoutput):
    ...
    if '200000' in database:
        max_penalty = 0.2
    if 'human' in args.i:
        cmds = '/usr/bin/time -v java -Xms850g -Xmx850g -jar %s  --max-penalty %s --num-threads %s --reference %s --queries %s  --queries %s --out-vcf %s.vcf\n' % (
            latest_mapper, max_penalty, min(40, args.t), database, files, files2, tempbamoutput)
    else:
        cmds = '/usr/bin/time -v java -Xms10g -Xmx10g -jar %s --max-penalty %s --num-threads %s --reference %s --queries %s  --queries %s --out-vcf %s.vcf\n' % (
            latest_mapper, max_penalty, min(40, args.t), database, files, files2, tempbamoutput)
    return cmds
...
fall = open(os.path.join(input_script, 'allsnpmodel.sh'), 'w')
fall.write('#!/bin/bash\nsource ~/.bashrc\n')
for i in range(0,10):
    output_dir = args.o + '/SNP_model_%s'%(i)
    if time_evaluation:
        input_script_sub = '%s/SNP_model_%s' % (input_script,i)
    else:
        input_script_sub = '%s/SNP_model_onetime_%s' % (input_script,i)
    if args.indel == 'False':
        output_dir += '_noindel'
    ...
    for m in range(0,total_test):
        for sub_scripts in glob.glob(os.path.join(input_script_sub, '*.mapper1.vcf.sh')):
            ...
            if 'human' in args.i:
                fall.write('jobmit %s%s %s%s big\n' % (sub_scripts,m,os.path.split(sub_scripts)[-1],m))
            else:
                fall.write('jobmit %s%s %s%s small\n' % (sub_scripts, m, os.path.split(sub_scripts)[-1], m))
fall.close()
