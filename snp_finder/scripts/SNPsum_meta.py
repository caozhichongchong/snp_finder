...
try:
    ...
    pass
# setup cutoff
min_sum_allele_freq = 3
min_separate_allele_freq = 2
################################################### Function ########################################################
__metaclass__ = type
...
class SNP_lineage:
    ...
    def addSNP(self,CHR_POS,Major_ALT,Minor_ALT,withinHS,allHS,genename):
        self.position.setdefault(CHR_POS,[Major_ALT,Minor_ALT,withinHS,allHS,genename,[],[]])
...
def compute_freq(freq1,freq2,freq3):
    ...
    if freq_sum >= min_sum_allele_freq:
...
def output_sum(sumfiles,output_list_within,output_list_across,output_list_other):
    ...
    for lines in open(sumfiles,'r'):
        if not lines.startswith("CHR\tPOS"):
            ...
            within_HS, HS_all = lines_set[7:9]
            newlines = '\t'.join(lines_set[0:4]) + '\t%s\t%s\t%s\t%s'%(lines_set[9],samplename,sum_freq,Tag)
            if within_HS == 'True':
...
for lineage in alllineage:
    ...
    for sumfiles in allsumfiles:
        output_sum(sumfiles, output_list_within,output_list_across,output_list_other)
