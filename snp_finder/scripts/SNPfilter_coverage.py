...
def SNP_filter(vcf_file,Sample_name,output_name,depth_ALT_cutoff, bowtie = True):
    ...
    for lines in open(vcf_file, 'r'):
        if not lines.startswith("#") and not lines.startswith("CHR"):
            ...
            if bowtie:
                CHR = lines_set[0]
                Depth = sum([int(i) for i in lines_set[7].split('DP4=')[1].split(';')[0].split(',')])
                Qual = float(lines_set[5])
            else:
                CHR = lines_set[0].split(' ')[0]
                Depth = float(lines_set[4])
                Qual = min_qual_for_call
            if Qual >= min_qual_for_call:
                genome = '_'.join(CHR.split('_')[:2])
                alldepth.setdefault(genome,[[],0])
                alldepth[genome][0].append(Depth)
                if lines_set[2] != '-':
                    alldepth[genome][1]+=1
    alloutput = []
    alloutput.append('genome\tsumdepth\tmean_depth\tdepth_0.1\tmedian_depth\tdepth_0.9\tgenome_coverage\n')
    for genome in alldepth:
        genomedepth = alldepth[genome][0]
        alloutput.append('%s\t%s\t%s\t%s\t%s\n'%(
            genome,sum(genomedepth),mean(genomedepth),'\t'.join(
                [str(i) for i in np.quantile(genomedepth, [0.1, 0.5, 0.9])]
            ),
            alldepth[genome][1]))
    f1 = open(vcf_file + '.%s.allfreqsum.txt' % (output_name), 'w')
    f1.write(
         ''.join(alloutput))
    f1.close()
    if False:
...
def load_sample_mapper(vcf_file):
    Sample_name = []
    mapper_sh = glob.glob(os.path.join(input_script,os.path.split(vcf_file)[-1].split('mapper')[0]
                                                                           +'*mapper*.sh'))
