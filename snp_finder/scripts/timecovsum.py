...
input_script = '/scratch/users/anniz44/scripts/1MG/donor_species/snp_curate/SNP_model_mapperkmer/16mer'
...
for errfile in glob.glob('%s/*.err'%(input_script)):
    ...
    filename = os.path.split(errfile)[-1].split('.SNP.fasta.')[0]
    if filename != 'SNPfilter.sh.err':
        ...
        if len(time_file)==4:
            ...
            alltime.append(
                '%s\t%s\t%.2f\t%.2f\n' % (fasta, SNP, time_file[0], time_file[1]))
            allmem.append(
                '%s\t%s\t%.2f\t%.2f\n' % (fasta, SNP, mem_file[0], mem_file[1]))
...
f1.write('fasta\tSNP\tmapper-xmer\tmapper-kmer\n')
