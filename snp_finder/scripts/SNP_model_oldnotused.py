...
def run_mapper(files,files2,database,tempbamoutput):
    if 'human' in args.i:
        cmds = '/usr/bin/time -v java -Xms850g -Xmx850g -jar %s/mapper1.31.jar --num-threads 40 --reference %s --queries %s  --queries %s --out-vcf %s.vcf\n' % (
        args.s, database, files, files2, tempbamoutput)
    else:
        cmds = '/usr/bin/time -v java -Xms220g -Xmx220g -jar %s/mapper1.31.jar --num-threads 40 --reference %s --queries %s  --queries %s --out-vcf %s.vcf\n' % (
            args.s, database, files, files2, tempbamoutput)
    return cmds
