...
output_gene = output_dir_merge + '/summary/all.selected.gene.across.lineage.faa'
compute_parallel = True # whether to find PE across lineages
...
def cluster_uc(cluster_input):
    ...
    for lines in open(cluster_input, 'r'):
        ...
        if compute_parallel:
            if record_name2!= '*':
                ...
                if species in ['BA','BL','PaDi'] and species == species2:
    ...
    newoutput = []
    newoutput.append('gene\tcluster\tspecies\tnum_donor\tdonor_list\n')
    if compute_parallel and High_select2_SNP!= dict():
        for record in High_select2_SNP:
            ...
            if len(donorset) >= Donor_species_cutoff.get(species,num_donor_cutoff):
                # at least num_donor_cutoff or more lineages
                High_select2.add(record)
                newoutput.append(
                    '%s\t%s\t%s\t%s\t%s\n'%(record,Clusters[record],species,len(donorset),';'.join(donorset))
                )
    f1 = open(output_gene + '.sum.txt', 'w')
...
def add_new_selection(input_summary,High_select2):
    ...
    for species in Species:
        ...
        newoutput = output_highlight(Species_notHS[species],
                                     line_set_sub, newoutput)
    f1 = open(input_summary + '.across.lineages.txt', 'w')
    f1.write(''.join(newoutput))
    f1.close()
    return High_select2
