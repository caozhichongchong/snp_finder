...
def sum_kegg_eggnog(Blast_outputegg,DB_nameegg,Clusters):
            ...
            for db_name in Blast_outputegg[gene_name]:
                if db_name in DB_nameegg:
                    for funegg in DB_nameegg[db_name]:
                        ...
                        BS_anno.setdefault(donor,'%s\t%s\t%s\n'%(gene_name_sub,db_name,funegg))
    ...
    for lines in open(BS_file,'r'):
        lines = lines.split('\n')[0]
        if lines.startswith('SNPdiff'):
            lines += 'gene_name\tEGGNOG\tannotation\tCOG\tCOG1\tCOG2\n'
        elif lines != '':
            seq = lines.split('\t')[2]
            seq_rc = str(Seq(seq).reverse_complement())
            if seq_rc in BS_anno:
                seq = seq_rc
            lines += BS_anno.get(seq,'\t\t\t\t\t\n')
...
for all_fasta in all_fasta_set:
    ...
    BS_file = all_fasta.replace('.faa.cluster.aa','.txt')
    print(BS_file)
    # sum up
